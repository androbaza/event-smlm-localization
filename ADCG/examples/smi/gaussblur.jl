# LEONARDO using SparseInverseProblems
using .SparseInverseProblems
# LEONARDO include("../../src/SparseInverseProblems.jl")
import .SparseInverseProblems: lmo, phi, solveFiniteDimProblem, localDescent,
getStartingPoint, parameterBounds, computeGradient
# LEONARDO using SparseInverseProblems.Util
using .SparseInverseProblems.Util
# LEONARDO include("../../src/util/ip_lasso.jl")
# LEONARDO include("../../src/util/ip.jl")
# LEONARDO
# LEONARDO include("../../src/BoxConstrainedDifferentiableModel.jl")

# LEONARDO immutable GaussBlur2D <: BoxConstrainedDifferentiableModel
...
function computeF!(f,sigma, u,l, x :: Float64)
  ...
  xovers = x/sigma
  # LEONARDO const prefactor = sigma * sqrt(pi) / 2.0
...
function computeFG(s :: GaussBlur2D, x :: Float64, f :: Vector{Float64}  = zeros(s.n_pixels), g :: Vector{Float64}  = zeros(s.n_pixels))
  ...
  xovers = x/s.sigma
  # LEONARDO const prefactor = s.sigma * sqrt(pi) / 2.0
